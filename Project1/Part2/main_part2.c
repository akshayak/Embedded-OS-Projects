...
void *threaddev1()
{
	...
	{	
		...
		{
			sprintf(buf, "%d%s%d", fileopCount1, " ", fileopCount1);
			res=write(fd, buf, strlen(buf)+1);
			printf("Invoking write operation for rbt530_dev1\n");
		}
		sleep(randnum);	
...
void *threaddev2()
{
	...
	{	
		...
		else
		{
			...
			res=write(fd, buf, strlen(buf)+1);
			printf("Invoking write operation for rbt530_dev2\n");
		}
		sleep(randnum);	
...
void *threadrbprobe(void *offs)
{
	...
	sprintf(buf, "%d%s%d%s%d", rw, " ", offset, " ",flag);

	//register kprobe for write function
	printf("Registering Kprobe for write through rbprobe\n");
...
int main(int argc, char **argv)
{
	...
	pthread_t tid2[2];
	pthread_t tid3;
	int i,ret,offset,fd,res;
	char readbuf[200];
	char buf[25];

	printf("Enter offset for kprobe");
	scanf("%d",&offset);
	...
	pthread_create(&tid3, &tattr, threadrbprobe, (void *)offset);
	...
	//read buffer for write function from rbprobe1
	printf("Reading from rbprobe for write function\n");
	fd = open("/dev/rbprobe", O_RDWR);
	if (fd < 0 ){
		printf("Can not open device file.\n");		
		return 0;
	}
	res=read(fd,readbuf,200);
	printf("Read:%s\n", readbuf);
	close(fd);

	//unregister rbprobe
	printf("Unregistering rbprobe for write function\n");
	int flag=0;
	int rw=1;
	sprintf(buf, "%d%s%d%s%d", rw, " ", offset, " ",flag);
	fd = open("/dev/rbprobe", O_RDWR);
